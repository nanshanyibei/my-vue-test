			...
			<el-table
				...
				<el-table-column
					...
					<template slot-scope="scope">
						<el-input v-if="scope.row.isEdit" v-model="scope.row.bankCard" />
						<span v-else style="margin-left: 10px">{{ scope.row.bankCard }}</span>
					</template>
				</el-table-column>
  ...
  data () {
	...
	methods: {
		handleEdit(index, row) {
			const element = {
				bankCard: this.tableData[index].bankCard,
				isEdit: true
			}
